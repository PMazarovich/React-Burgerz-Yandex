...
// Constants representing the different states of the WebSocket connection
...
// Mapping of WebSocket readyState codes to their corresponding states
...
export const useSocket = (url: string, options: IWSOptions) => {
    ...
    const connect = useCallback(
        (token: string) => {
            // присвоили рефу инстанс WebSocket
            ws.current = new WebSocket(`${url}?token=${token}`); // Create a new WebSocket instance with a token in the URL
